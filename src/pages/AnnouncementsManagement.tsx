...
const TYPE_META: Record<AnnouncementType, { label: string; emoji: string; color: string; description: string }> = {
  ...
  promotion:   { label: 'Promotion',     emoji: '🎁', color: '#C9A96E', description: 'Deals & promotions'   },
  closure:     { label: 'Closure',       emoji: '⚠️', color: '#EF4444', description: 'Closures & rescheduling' },
  menu_update: { label: 'Menu Update',   emoji: '🍽️', color: '#2C5530', description: 'Menu changes'         },
  community:   { label: 'Community',     emoji: '🤝', color: '#243A7D', description: 'Community events'     },
  holiday:     { label: 'Holiday',       emoji: '🎄', color: '#9B8B80', description: 'Holiday notices'      },
};

const PRIORITY_META: Record<AnnouncementPriority, { label: string; color: string }> = {
  low:    { label: 'Low',    color: '#9B8B80' },
  normal: { label: 'Normal', color: '#2C5530' },
  high:   { label: 'High',   color: '#C9A96E' },
  urgent: { label: 'Urgent', color: '#BE5953' },
};

const STATUS_META: Record<AnnouncementStatus, { label: string; color: string; bg: string }> = {
  draft:   { label: 'Draft',     color: '#9B8B80', bg: 'rgba(155,139,128,0.1)' },
  sending: { label: 'Sending…',  color: '#243A7D', bg: 'rgba(36,58,125,0.08)'  },
  sent:    { label: 'Sent',      color: '#2C5530', bg: 'rgba(44,85,48,0.08)'   },
...
const AnnouncementsManagement: React.FC = () => {
  ...
  const summaryStats: StatItem[] = [
    ...
    { label: 'Sent',       value: stats.sent,            icon: <SentIcon />,     color: '#2C5530' },
    { label: 'Drafts',     value: stats.drafts,          icon: <DraftsIcon />,   color: '#C9A96E' },
    { label: 'Recipients', value: stats.totalRecipients, icon: <GroupIcon />,    color: '#243A7D' },
  ];
  ...
  return (
    <Box>
        ...
        action={
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            <Tooltip title="Refresh">
              <IconButton onClick={fetchData} sx={{ color: '#BE5953', border: '1px solid #E8E0D8', borderRadius: '5px' }}>
      ...
      {loading && <LinearProgress sx={{ mb: 2, height: 2, bgcolor: '#F0EBE4', '& .MuiLinearProgress-bar': { bgcolor: '#BE5953' } }} />}
      ...
      >
            ...
            <Button
              ...
              sx={{
                ...
                ...(filterStatus === s
                  ...
                  : { color: '#6B5C52', '&:hover': { bgcolor: '#F5F0EB' } }),
              }}
            >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <TextField
            ...
            sx={{ width: 240, ...fieldSx, '& .MuiOutlinedInput-root': { borderRadius: '5px', fontSize: '0.845rem' } }}
          />
          <Typography sx={{ fontSize: '0.78rem', color: '#9B8B80', whiteSpace: 'nowrap' }}>
        ...
        <TableContainer>
          <Table size="small">
            ...
            <TableBody>
              {filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} sx={{ textAlign: 'center', py: 6, color: '#9B8B80', fontSize: '0.875rem' }}>
                  ...
                  return (
                    <TableRow
                      ...
                      sx={{
                        ...
                        '&:hover': { bgcolor: '#FDFAF8' },
                        '&:last-child td': { borderBottom: 'none' },
                      }}
                    >
                      <TableCell sx={{ px: 2, py: 1.5, maxWidth: 300 }}>
                        <Typography sx={{ fontWeight: 600, fontSize: '0.845rem', color: '#1C1917', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {a.title}
                        </Typography>
                        <Typography sx={{ fontSize: '0.75rem', color: '#9B8B80', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 280 }}>
                          {a.content}
                        </Typography>
                        {a.sentAt && (
                          <Typography sx={{ fontSize: '0.68rem', color: '#9B8B80', mt: 0.25 }}>
                      ...
                      <TableCell sx={{ px: 2, py: 1.5, whiteSpace: 'nowrap' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                          ...
                          <Typography sx={{ fontSize: '0.8rem', color: '#5C524D' }}>{type.label}</Typography>
                        </Box>
                      </TableCell>
                      ...
                      <TableCell sx={{ px: 2, py: 1.5, whiteSpace: 'nowrap' }}>
                        <Typography sx={{ fontSize: '0.78rem', color: '#9B8B80' }}>
                      ...
                      <TableCell sx={{ px: 2, py: 1.5 }}>
                        <Box sx={{ display: 'flex', gap: 0.5 }}>
                          {a.status === 'draft' && (
                            <>
                              <Tooltip title="Edit">
                                <IconButton size="small" onClick={() => openEdit(a)} sx={{ color: '#9B8B80', '&:hover': { color: '#BE5953', bgcolor: 'rgba(190,89,83,0.08)' } }}>
                                  <EditIcon sx={{ fontSize: 15 }} />
                                </IconButton>
                              </Tooltip>
                              <Tooltip title="Send to subscribers">
                                <IconButton size="small" onClick={() => { setSelectedAnnouncement(a); setSendDialogOpen(true); }} sx={{ color: '#9B8B80', '&:hover': { color: '#2C5530', bgcolor: 'rgba(44,85,48,0.08)' } }}>
                          ...
                          <Tooltip title="Delete">
                            <IconButton size="small" onClick={() => { setSelectedAnnouncement(a); setDeleteDialogOpen(true); }} sx={{ color: '#9B8B80', '&:hover': { color: '#EF4444', bgcolor: 'rgba(239,68,68,0.08)' } }}>
      ...
      >
        ...
        >
          ...
          <IconButton size="small" onClick={() => setFormOpen(false)} sx={{ color: '#9B8B80' }}>
        ...
        <DialogContent sx={{ pt: '16px !important' }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            {/* Type selector */}
            <Box>
              <Typography sx={{ fontSize: '0.78rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#9B8B80', mb: 1.25 }}>
                Announcement Type
              </Typography>
                  ...
                  <Grid size={{ xs: 6, sm: 4 }} key={key}>
                    <Box
                      ...
                      sx={{
                        ...
                        p: 1.5,
                        borderRadius: '5px',
                        border: `2px solid ${formData.type === key ? meta.color : '#E8E0D8'}`,
                    ...
                    >
                      ...
                      <Box>
                        <Typography sx={{ fontSize: '0.8rem', fontWeight: 700, color: '#1C1917' }}>{meta.label}</Typography>
                        <Typography sx={{ fontSize: '0.68rem', color: '#9B8B80' }}>{meta.description}</Typography>
                      </Box>
                    </Box>
                  </Grid>
                ))}
            ...
            <FormControl size="small" sx={{ maxWidth: 180 }}>
              ...
              <Select value={formData.priority} label="Priority" onChange={(e) => setFormData((p) => ({ ...p, priority: e.target.value as AnnouncementPriority }))} sx={{ borderRadius: '5px' }}>
        ...
        <DialogActions sx={{ px: 2.5, pb: 2.5, gap: 1, borderTop: '1px solid #F0EBE4', pt: 2 }}>
          <Button onClick={() => setFormOpen(false)} variant="outlined" sx={{ borderColor: '#E8E0D8', color: '#6B5C52', borderRadius: '5px', fontWeight: 600, textTransform: 'none', '&:hover': { borderColor: '#BE5953', color: '#BE5953', bgcolor: 'transparent' } }}>
            Cancel
          </Button>
          <Button
            onClick={handleSave}
            ...
            sx={{ bgcolor: '#BE5953', '&:hover': { bgcolor: '#A84E48' }, borderRadius: '5px', fontWeight: 700, textTransform: 'none', boxShadow: 'none' }}
          >
      ...
      <Dialog
        ...
        slotProps={{ paper: { sx: { borderRadius: '8px', border: '1px solid #E8E0D8', maxWidth: 480 } } }}
      >
        <DialogTitle sx={{ fontWeight: 700, fontSize: '1rem', color: '#1C1917', borderBottom: '1px solid #F0EBE4', pb: 2, display: 'flex', alignItems: 'center', gap: 1.25 }}>
          <Box sx={{ width: 30, height: 30, borderRadius: '6px', bgcolor: 'rgba(44,85,48,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <SendIcon sx={{ fontSize: 15, color: '#2C5530' }} />
          </Box>
          Send Announcement
        </DialogTitle>
        <DialogContent sx={{ pt: '16px !important' }}>
          {selectedAnnouncement && (
            <Box>
              <Typography sx={{ fontSize: '0.875rem', color: '#5C524D', mb: 2 }}>
                Send this announcement to <strong>all active subscribers</strong>?
              </Typography>
              <Box sx={{ p: 2, bgcolor: '#FDFAF8', border: '1px solid #E8E0D8', borderRadius: '6px' }}>
                ...
                <Typography sx={{ fontSize: '0.8rem', color: '#9B8B80', mt: 0.5 }}>
              ...
              {(selectedAnnouncement.priority === 'high' || selectedAnnouncement.priority === 'urgent') && (
                <Fade in>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 2, p: 1.5, bgcolor: 'rgba(201,169,110,0.08)', border: '1px solid rgba(201,169,110,0.25)', borderRadius: '5px' }}>
                    <WarningIcon sx={{ fontSize: 16, color: '#C9A96E', flexShrink: 0 }} />
                    <Typography sx={{ fontSize: '0.8rem', color: '#6B5C52' }}>
        ...
        <DialogActions sx={{ px: 2.5, pb: 2.5, gap: 1, borderTop: '1px solid #F0EBE4', pt: 2 }}>
          <Button onClick={() => setSendDialogOpen(false)} disabled={sending} variant="outlined" sx={{ borderColor: '#E8E0D8', color: '#6B5C52', borderRadius: '5px', fontWeight: 600, textTransform: 'none', '&:hover': { borderColor: '#BE5953', color: '#BE5953', bgcolor: 'transparent' } }}>
            Cancel
          </Button>
          <Button
            onClick={handleSend}
            ...
            sx={{ bgcolor: '#2C5530', '&:hover': { bgcolor: '#224027' }, borderRadius: '5px', fontWeight: 700, textTransform: 'none', boxShadow: 'none' }}
          >
      ...
      <Dialog
        ...
        slotProps={{ paper: { sx: { borderRadius: '8px', border: '1px solid #E8E0D8', maxWidth: 420 } } }}
      >
        <DialogTitle sx={{ fontWeight: 700, fontSize: '1rem', color: '#1C1917', borderBottom: '1px solid #F0EBE4', pb: 2, display: 'flex', alignItems: 'center', gap: 1.25 }}>
          <Box sx={{ width: 30, height: 30, borderRadius: '6px', bgcolor: 'rgba(239,68,68,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <DeleteIcon sx={{ fontSize: 15, color: '#EF4444' }} />
          </Box>
          Delete Announcement
        </DialogTitle>
        <DialogContent sx={{ pt: '16px !important' }}>
          <Typography sx={{ fontSize: '0.875rem', color: '#5C524D' }}>
            Are you sure you want to permanently delete <strong>"{selectedAnnouncement?.title}"</strong>? This action cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 2.5, pb: 2.5, gap: 1, borderTop: '1px solid #F0EBE4', pt: 2 }}>
          <Button onClick={() => setDeleteDialogOpen(false)} variant="outlined" sx={{ borderColor: '#E8E0D8', color: '#6B5C52', borderRadius: '5px', fontWeight: 600, textTransform: 'none', '&:hover': { borderColor: '#BE5953', color: '#BE5953', bgcolor: 'transparent' } }}>
            Cancel
          </Button>
          <Button onClick={handleDelete} variant="contained" sx={{ bgcolor: '#EF4444', '&:hover': { bgcolor: '#DC2626' }, borderRadius: '5px', fontWeight: 700, textTransform: 'none', boxShadow: 'none' }}>
            Delete
          </Button>
        </DialogActions>
      </Dialog>
