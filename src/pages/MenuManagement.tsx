...
const MenuManagement: React.FC = () => {
  ...
  const [filterPrimaryCategory, setFilterPrimaryCategory] =
    useState<string>('all');
  ...
  const filteredCategories = useMemo(() => {
    ...
    if (filterPrimaryCategory !== 'all') {
      result = result.filter(
        (cat) => cat.primaryCategoryId === filterPrimaryCategory,
      );
    }
      ...
      if (response.ok) {
        ...
        if (primaryImagesToDelete.length > 0) {
          try {
            ...
            logger.error('Error deleting primary category images:', err);
          }
        }
    ...
    try {
      ...
      if (itemForm.hasMeasurements) {
        if (itemForm.measurements.length === 0) {
          showSnackbar(
            'At least one measurement is required when measurements are enabled',
            'error',
          );
          return;
        }
        for (const measurement of itemForm.measurements) {
          if (
            ...
            !measurement.measurementTypeId.trim()
          ) {
          ...
          if (measurement.price < 0) {
            showSnackbar(
              'All measurement prices must be 0 or greater',
              'error',
            );
            return;
          }
  ...
  const primaryCategoryColumns: GridColDef[] = [
    {
      ...
      renderCell: (params) =>
        params.value ? (
          ...
          >
            <ImageIcon sx={{ color: '#BE5953', opacity: 0.5, fontSize: 24 }} />
          </Box>
        ),
    },
    {
          ...
          <Tooltip title="Move Up" arrow>
            <span>
              <IconButton
                size="small"
                onClick={() =>
                  handleMovePrimaryCategoryOrder(params.row.id, 'up')
                }
          ...
          <Tooltip title="Edit" arrow>
            <IconButton
              ...
              sx={{
                ...
                width: 32,
                height: 32,
                '&:hover': { bgcolor: 'rgba(190, 89, 83, 0.15)' },
              }}
            >
            ...
            <IconButton
              ...
              onClick={() =>
                setDeleteTarget({
                  id: params.row.id,
                  type: 'category',
                  label: 'category and all its items',
                })
              }
              sx={{
                color: '#EF5350',
        ...
        if (hasMeasurements && measurements.length > 0) {
          return (
            <Box sx={{ display: 'flex', gap: 0.75, flexWrap: 'wrap', py: 0.5 }}>
              {measurements
                .sort(
                  (a: MenuItemMeasurement, b: MenuItemMeasurement) =>
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (params.row.isGlutenFree)
          tags.push({ label: 'GF', color: '#FF9800', title: 'Gluten Free' });
        if (params.row.isDairyFree)
          tags.push({ label: 'DF', color: '#03A9F4', title: 'Dairy Free' });
    ...
    <Box sx={{ minHeight: '100vh' }}>
        ...
        >
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid
              size={{
                xs: 12,
                md: activeTab === 2 ? 4 : activeTab === 1 ? 5 : 12,
              }}
            >
              <TextField
                size="small"
                ...
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">
                      <SearchIcon
                        sx={{ color: '#BE5953', fontSize: '1.2rem' }}
                      />
                    </InputAdornment>
                  ),
                }}
                sx={{
                  '& .MuiOutlinedInput-root': {
                    ...
                    '&.Mui-focused': {
                      backgroundColor: 'white',
                      boxShadow: '0 2px 8px rgba(190, 89, 83, 0.12)',
                      '& fieldset': {
                        borderColor: '#BE5953',
                        borderWidth: '1.5px',
                      },
                ...
                >
                  ...
                  >
                    <MenuItem value="all">
                      ...
                      >
                        <FolderIcon sx={{ fontSize: 18, color: '#BE5953' }} />
          ...
          {((activeTab === 1 && filterPrimaryCategory !== 'all') ||
            ...
            <Box sx={{ mt: 1.5, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              <Typography
                variant="caption"
                sx={{ color: '#6B4E3D', alignSelf: 'center', mr: 0.5 }}
              >
                Active filters:
              </Typography>
            ...
            <DataGrid
              ...
              sx={{
                ...
                '& .MuiDataGrid-columnHeaders': {
                  ...
                  borderBottom: '2px solid rgba(190, 89, 83, 0.15)',
                  minHeight: 52,
                ...
                '& .MuiDataGrid-footerContainer': {
                  ...
                  borderTop: '1px solid rgba(190, 89, 83, 0.1)',
                },
                '& .MuiTablePagination-root': {
                  color: '#6B4E3D',
                },
                '& .MuiTablePagination-selectIcon': { color: '#BE5953' },
            ...
            <Grid size={{ xs: 12 }}>
              <Box sx={{ mt: 2, mb: 2 }}>
                ...
                <Typography
                  ...
                  display="block"
                >
                  Enable this to add different sizes (Small, Medium, Large,
                  Pint, Pitcher, etc.) with individual pricing
                </Typography>
              </Box>
                          ...
                          <FormControl fullWidth size="small">
                            ...
                            <Select
                              ...
                              onChange={(e) => {
                                ...
                                newMeasurements[index].measurementTypeId = e
      ...
      >
        ...
        <DialogContent sx={{ px: 3, py: 3 }}>
          <Typography
            sx={{ color: '#6B4E3D', fontSize: '0.938rem', lineHeight: 1.6 }}
          >
            Are you sure you want to delete this {deleteTarget?.label}? This
          ...
          <Button
            onClick={() => {
              ...
              if (deleteTarget.type === 'primary')
                handleDeletePrimaryCategory(deleteTarget.id);
